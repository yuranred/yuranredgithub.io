...
<div id="page" class="home-page">
	<div class="content">
		<div class="article"> 
			<h1 class="postsby">
				<?php if (is_category()) { ?>
					<span><?php _e("Category: ", "czs"); ?><?php single_cat_title(); ?></span>
				<?php } elseif (is_tag()) { ?> 
					<span><?php _e("Tag: ", "czs"); ?><?php single_tag_title(); ?></span>
				<?php } elseif (is_search()) { ?> 
					<span><?php _e("Search Results for:", "czs"); ?></span> <?php the_search_query(); ?>
				<?php } elseif (is_author()) { ?>
					<span><?php  $curauth = (isset($_GET['author_name'])) ? get_user_by('slug', $author_name) : get_userdata(intval($author)); echo $curauth->nickname; _e(" Archive", "czs"); ?></span> 
				<?php } elseif (is_day()) { ?>
					<span><?php _e("Daily Archive:", "czs"); ?></span> <?php the_time('l, F j, Y'); ?>
				<?php } elseif (is_month()) { ?>
					<span><?php _e("Monthly Archive:", "czs"); ?></span> <?php the_time('F Y'); ?>
				<?php } elseif (is_year()) { ?>
					<span><?php _e("Yearly Archive:", "czs"); ?></span> <?php the_time('Y'); ?>
				<?php } ?>
			</h1>	
